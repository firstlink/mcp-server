...
def truncate_description(description: str, max_length: int = 1200) -> str:
    """
    Truncate description to meet AWS Bedrock constraints.
    
    Args:
        description: The description to truncate
        max_length: Maximum allowed length (default: 1200 for function descriptions)
        ...
        for tool in tools_list:
            if len(tools_to_use) != 0:
                if tool.name in tools_to_use:
                    function = {
                        "description": truncate_description(tool.description, 1200),
                        "name": tool.name,
                        "parameters": {},
                        ...
                        for param_name, param_details in tool.inputSchema[
                            ...
                            function["parameters"][param_name] = {
                                "description": truncate_description(
                                    param_details.get("description", param_name), 500
                                ),
